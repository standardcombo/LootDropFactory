...
function API.Drop(eventData)
	...
	if target then
		...
		if encounterData then
			...
			for _, player in ipairs(players) do
				if player then
					encounterPlayers[player] = player
				end
			end
			...
			if #encounterNPCs > 0 then
				...
				local _lootId = encounterData.lootId
				if _lootId and _lootId ~= "" then
					eventData.lootId = _lootId
				end
			end
		end
	end
	...
	-- Loot drops
	local playersToReward = players
	if encounterData then
		playersToReward = encounterData.players
	end
	for _,player in pairs(playersToReward) do
...
function API.RegisterEncounter(encounterNpcs, lootDropId, level)
	...
	local encounterData = {}
	activeEncounters[auto_encounterId] = encounterData
...
function OnNpcDestroyed(npc)
	...
	local encounterData = activeEncounters[encounterId]
	for i,obj in ipairs(encounterData.npcs) do
		if npc == obj then
			table.remove(encounterData.npcs, i)
			break
		end
	end
end
